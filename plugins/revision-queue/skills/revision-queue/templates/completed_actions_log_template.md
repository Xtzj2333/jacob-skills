...
- **Working-doc invariant.** Once an entry lands in this log, the source row in the actions file (for ACTIONs, 3-file async pattern only) or the todos file (for TODOs) is removed. The log is the single source of truth for resolved items; the working docs only show open items.

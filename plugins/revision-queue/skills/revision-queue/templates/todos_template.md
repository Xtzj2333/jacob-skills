---
title: "<PROJECT> — <USER>'s To-Do List"
...
- **<USER>'s response** — fill this in; I'll read and act on it (or comment in the docx margin)

Just type your decision under "<USER>'s response" for any item. Strike through, add new items, or rearrange — I'll pick up whatever's there.
...
**Lifecycle.** When a TODO gets resolved (either by approving a code edit or by inaction/compliment/prior fix), it is **removed from this file** and appended to `completed_actions_log.md` with a TYPE tag. This file shows only **open** items; the change log is the audit trail. Code edits that are approved but not yet executed sit in `<USER>_actions.md` until a fresh Claude session runs them.

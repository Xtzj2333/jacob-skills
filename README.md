...
| `chrome-tab` | Opens an HTML file or URL in the right Chrome window **and tab group** without stealing focus — it never changes which tab a window is showing either (0.5.0), and never opens a window you didn't ask for — fixes bare `open` landing the tab in whichever window was used last and yanking Chrome in front of what you were doing (`open -g` does not help; Chrome activates regardless). Tab groups, a real tab move and a quiet open path come from an optional helper extension (`chrome-tab helper install`, one Load-unpacked click); an optional hook moves Claude-in-Chrome's browsing tabs beside the session's pages. Also ships an optional `PreToolUse` hook that refuses `open <file>.html`. **macOS + Google Chrome only** — after installing, start a new session: the plugin puts the command on your PATH and runs `chrome-tab doctor`. |
